...
$regressionGate = Join-Path $PSScriptRoot 'phase-three-four-gate.ps1'
...
& $regressionGate -ReuseFlutterPackages:$ReuseFlutterPackages
if ($LASTEXITCODE -ne 0) {
    throw 'Phase-one through phase-four regression gate failed.'
}
...
try {
    ...
    Invoke-Compose exec -T backend `
        bench --site $siteName execute asoud_core.phase_five_acceptance.run_phase_five_acceptance
    Invoke-Compose exec -T frontend curl -fsS http://localhost:8080/api/method/ping
} finally {
    Pop-Location
}

Write-Host 'ASOUD ERP phase-five gate passed.'
